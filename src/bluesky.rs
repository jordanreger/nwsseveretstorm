...
pub async fn post_to_bluesky(warning_text: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let auth = BskyUser {
        identifier: dotenv!("NWSSEVERETSTORM_IDENTIFIER").into(),
        password: dotenv!("NWSSEVERETSTORM_PASSWORD").into(),
    };
    ...
    let post = PostRecord {
        ...
        repo: "nwsseveretstorm.bsky.social".to_string(),
        collection: "app.bsky.feed.post".to_string(),
        record: body,
    };

    let _post_send = client
        .post("https://bsky.social/xrpc/com.atproto.repo.createRecord?repo=nwsseveretstorm.bsky.social")
